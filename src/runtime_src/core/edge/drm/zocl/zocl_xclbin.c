...
zocl_xclbin_load_pdi(struct drm_zocl_dev *zdev, void *data)
{
	...
	if (zocl_xclbin_same_uuid(zdev, &axlf_head->m_header.uuid)) {
		DRM_INFO("The XCLBIN already loaded, uuid: %pUb. "
		    "Don't need to reload.", &axlf_head->m_header.uuid);
		return ret;
	}
	...
	/*XXX need block comment */
	zdev->zdev_xclbin->zx_flag = ZOCL_XCLBIN_DYNAMIC;
...
zocl_xclbin_read_axlf(struct drm_zocl_dev *zdev, struct drm_zocl_axlf *axlf_obj)
{
	...
	/*
	 * Remember xclbin_uuid for opencontext and double download.
	 */
...
zocl_xclbin_init(struct drm_zocl_dev *zdev)
{
	...
	zdev->zdev_xclbin->zx_flag = ZOCL_XCLBIN_DEFAULT;
